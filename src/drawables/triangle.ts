...
export class Triangle implements IDrawable {
  ...
  public defaultAnimation(point: Vector): Triangle {
    return new Triangle(
      this.point0.rotateInYAround(point, (2 * Math.PI) / 100),
      this.point1.rotateInYAround(point, (2 * Math.PI) / 100),
      this.point2.rotateInYAround(point, (2 * Math.PI) / 100),
      this.colour,
      this.update
    );
  }
  ...
  private getNormal(): Vector {
    return this.point1
      .subtract(this.point0)
      .crossProduct(this.point2.subtract(this.point0));
  }
}
